...
1. Native Binary Packaging Issue (2024-03-xx)
   ...
   - Fix: Added explicit `npx node-pre-gyp rebuild` step before packaging in the release workflow
   - Proper build sequence: build-rust-release → npx node-pre-gyp rebuild → package
...
3. Rebuild native module: `npx node-pre-gyp rebuild`
